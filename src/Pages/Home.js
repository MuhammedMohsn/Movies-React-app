...
function Home() {
  ...
  useEffect(() => {
    ...
    getUpComingMovies().then((data) =>{setLoading(false); setComingMovies(data)} )}, [])
          ...
          {trendingMovies.slice(0, 6).map((movie) => {
            ...
            return (<Col key={id} xs={4} md={3} lg={2} className={`${styles.col} mb-3`}>
              <Card className="w-100 h-100">
                <Card.Img variant="top" className='h-75' style={{ cursor: "pointer" }} onClick={() => {navigate(`/movies/${id}`) }} src={`https://image.tmdb.org/t/p/w500${poster_path}`} />
       ...
       {/**display top rated movies */}
        ...
        <Row>
          {trendingTv.slice(0, 6).map((serie) => {
            ...
            return (<Col key={id} xs={4} md={3} lg={2} className={`${styles.col} mb-3`} >
              <Card className="w-100 h-100">
                <Card.Img variant="top" className='h-75' style={{ cursor: "pointer" }} onClick={() => { navigate(`/tv/${id}`) }} src={`https://image.tmdb.org/t/p/w500${poster_path}`} />
                <Card.Body className='h-25'>
                  <Card.Text className='h-100'>
