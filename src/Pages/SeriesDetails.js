...
function SeriesDetails() {
  ...
  useEffect(() => {
    getSeriesById(id).then((seriedata) => {
      setSingleSeries(seriedata);
      setLoading(false);
    });
    getSeriesVideos(id).then((videosdata) => {
      setVideos(videosdata);
    });
    getSeriesCasts(id).then((castsdata) => {
      setCasts(castsdata);
    });
    getSeriesSimilar(id).then((similarseriesdata) => {
      setSimilar(similarseriesdata);
    });
  }, [id]);
  ...
  return (
    <Fragment>
      <div
        className={`${styles.backdrop} mb-5`}
        style={{
            ...
            >
              ...
              <div className={`${styles.badges}  w-100`}>
                {genres.map((genre) => {
                  ...
                  return (
                    <div key={Math.random()}>
                      <span bg="dark" className=" text-center ">
                        {" "}
                        {name}
                      </span>
                    </div>
              ...
              <div
                className="w-100 "
                style={{ height: "fit-content", fontSize: "2vw" }}
              >
      ...
      <Container>
        <Row>
          {videos.slice(0, 4).map((video) => {
            ...
            return (
              <Col key={id} xs={12} className="mb-3">
                <iframe
        ...
        <Row>
          {similar.slice(0, 5).map((movie) => {
            ...
            return (
              <Col xs={3} key={Math.random()} className="mb-4">
