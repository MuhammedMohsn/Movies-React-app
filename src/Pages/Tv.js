...
function Tv() {
  ...
  useEffect(() => {
    getOnTheAirAeries(page).then(seriesdata =>{setLoading(false);   setSeries(prevState => { return prevState.concat(seriesdata) })})
  }, [page])
          ...
          return (
            <Col xs={4} md={3} lg={2} key={Math.random()} className="mb-2">
              <Card style={{ width: '100%', height: "100%" }} >
                <Card.Img variant="top" onClick={() => {navigate(`/tv/${id}`) }} src={`https://image.tmdb.org/t/p/w500${poster_path}`} className={`${styles.movie_img}`} />
