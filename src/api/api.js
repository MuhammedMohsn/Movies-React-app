...
export let getMovieById = async (id) => {
  try {
    let response = await axios.get(`${URL}/movie/${id}?api_key=${API_key}`);
...
export let getSeriesById = async (id) => {
  try {
    let response = await axios.get(`${URL}/tv/${id}?api_key=${API_key}`);
    let data = await response.data;
    return data;
  } catch (err) {
    console.log(err);
...
export let getSeriesVideos = async (id) => {
  try {
    let response = await axios.get(`${URL}/tv/${id}/videos?api_key=${API_key}`);
    let data = await response.data;
    let results = data.results;
    return results;
  } catch (err) {
...
export let getTrendingTv = async (page = 1) => {
  try {
    let response = await axios.get(
      `${URL}/tv/popular?api_key=${API_key}&page=${page}`
    );
    let data = await response.data;
    let results = data.results;
    return results;
...
export let getTopRatedTv = async (page = 1) => {
  try {
    let response = await axios.get(
      `${URL}/tv/top_rated?api_key=${API_key}&page=${page}`
    );
    let data = await response.data;
    let results = data.results;
    return results;
